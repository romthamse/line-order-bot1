...
const NUMBER_WORDS = {
  ...
  '\u0e40\u0e01\u0e49\u0e32': 9, // \u0e40\u0e01\u0e49\u0e32
  '\u0e2a\u0e34\u0e1a': 10, // \u0e2a\u0e34\u0e1a
  '\u0e19\u0e36\u0e07': 1, // \u0e19\u0e36\u0e07 \u2014 colloquial short form of \u0e2b\u0e19\u0e36\u0e48\u0e07 ("one"); also
  // shows up glued onto \u0e16\u0e31\u0e07 ("keg") with no space as \u0e16\u0e31\u0e07\u0e19\u0e36\u0e07,
  // meaning "a keg" / "one keg" \u2014 substring matching here picks that up too.
};
...
/**
 * Splits a message into separate order "clauses" on common hard separators
 * people use when listing more than one item in a single message: commas,
 * semicolons, and line breaks \u2014 each of these starts a fresh, independent
 * item (and can carry its own date, e.g. one per line). "\u0e25\u0e32\u0e40\u0e01\u0e2d\u0e23\u0e4c x 1, \u0e42\u0e23\u0e40\u0e0b\u0e48 x 2"
 * -> ["\u0e25\u0e32\u0e40\u0e01\u0e2d\u0e23\u0e4c x 1", "\u0e42\u0e23\u0e40\u0e0b\u0e48 x 2"].
 *
 * "and" / "\u0e41\u0e25\u0e30" / "\u0e01\u0e31\u0e1a" (Thai for "and"/"with") are deliberately *not* split
 * on here \u2014 "one of Lager and rose" needs "Lager" and "rose" to stay in the
 * same clause so the shared "one" in front of both of them can be applied
 * to both; matchProductsInClause below handles "and" as glue between two
 * product mentions instead of a hard boundary between them.
 */
...
// Recognizes text that's *just* a conjunction ("and", "\u0e41\u0e25\u0e30", "\u0e01\u0e31\u0e1a") with
// nothing else in it \u2014 used to tell "Lager and rose" (glue only) apart from
// "Lager, then rose" or "Lager 2 rose" (real content in between), so a
// shared quantity only ever spans a plain "X and Y and Z" list.
const CONJUNCTION_ONLY_RE = /^\s*(?:and|\u0e41\u0e25\u0e30|\u0e01\u0e31\u0e1a)\s*$/i;

function isJustConjunction(text) {
  return CONJUNCTION_ONLY_RE.test(text);
}
 ...
 * another's) collapsed down to whichever was found first (a longer keyword
 * always wins over a shorter one sitting inside it, e.g. "\u0e40\u0e1a\u0e35\u0e22\u0e23\u0e4c\u0e14\u0e33" wins
 * over the bare color word "\u0e14\u0e33" that's also technically inside it).
 *
 * Short codes (config's `shortCodes`, e.g. "L"/"R"/"D") are matched
 * separately from ordinary keywords: only as a whole standalone word
 * (`\bL\b`), never as a substring \u2014 a single letter would otherwise match
 * constantly inside ordinary words ("please", "at", ...) if treated like
 * every other keyword. They're matched case-insensitively (so "l"/"r"/"d"
 * work too, not just "L"/"R"/"D") \u2014 which does mean an unrelated lowercase
 * "r" or "l" standing alone in ordinary chat right before a number could in
 * theory be misread as an order; the standalone-word requirement and the
 * "a mention needs an explicit quantity" rule together keep this rare.
 *
 * `strictAfterKeywords` (e.g. the color words "\u0e40\u0e2b\u0e25\u0e37\u0e2d\u0e07"/"\u0e41\u0e14\u0e07"/"\u0e14\u0e33" standing
 * in for Lager/Rose/Dunkel) are matched like ordinary keywords but flagged
 * `strict: true`, which matchProductsInClause uses to only ever accept a
 * quantity that comes right *after* the word \u2014 never a quantity borrowed
 * from before it or from a shared "X and Y" group. Colors come up in chat
 * for all sorts of reasons having nothing to do with ordering, so this
 * keeps a bare "\u0e14\u0e33" from ever being read as an order \u2014 only "\u0e14\u0e33 2" is.
 */
  ...
  for (const product of products) {
    for (const keyword of product.keywords) {
      ...
      while ((idx = lowerText.indexOf(lowerKeyword, searchFrom)) !== -1) {
        occurrences.push({ product, start: idx, end: idx + lowerKeyword.length, strict: false });
        searchFrom = idx + lowerKeyword.length;
      }
    }

    for (const keyword of product.strictAfterKeywords || []) {
      const lowerKeyword = keyword.toLowerCase();
      if (!lowerKeyword) continue;
      let searchFrom = 0;
      let idx;
      while ((idx = lowerText.indexOf(lowerKeyword, searchFrom)) !== -1) {
        // A strict color word glued directly onto the end of an unrelated
        // preceding Thai word (e.g. "\u0e04\u0e32\u0e23\u0e32\u0e1a\u0e32\u0e27" + "\u0e41\u0e14\u0e07", the real drink
        // brand "\u0e04\u0e32\u0e23\u0e32\u0e1a\u0e32\u0e27\u0e41\u0e14\u0e07") isn't the standalone color word at
        // all \u2014 skip this occurrence, but keep scanning past it in case the
        // same word shows up again later, standalone, elsewhere in the text.
        const precedingChar = idx > 0 ? text[idx - 1] : '';
        if (precedingChar && THAI_CHAR_PATTERN.test(precedingChar)) {
          searchFrom = idx + lowerKeyword.length;
          continue;
        }
        occurrences.push({ product, start: idx, end: idx + lowerKeyword.length, strict: true });
        searchFrom = idx + lowerKeyword.length;
      }
    }

    for (const code of product.shortCodes || []) {
      if (!code) continue;
      const codeRe = new RegExp(`\\b${escapeRegex(code)}\\b`, 'gi');
      let m;
      while ((m = codeRe.exec(text)) !== null) {
        // Short codes are marked strict for the same reason as the color
        // words: a single standalone letter is common enough elsewhere in
        // text (URLs, abbreviations, initials) that it must never fire
        // without an immediately-adjacent number, and never borrow one
        // from before it or from a shared "X and Y" group. This is what
        // keeps a link like ".../share/r/1BQphSts6w/" from being misread as
        // "R 1" \u2014 the "1" there isn't actually next to the "r", it's just
        // somewhere later in the same string.
        occurrences.push({ product, start: m.index, end: m.index + m[0].length, strict: true });
      }
    }
  }
...
function extractPrecedingQuantity(text) {
  ...
  // "1 x lager" / "1x lager" \u2014 the same "x" multiplier people also put
  // after the name ("lager x 1") works in front of it too. This only
  // strips an "x" that directly follows a digit (with or without a space),
  // so it can never eat into a spelled-out word ending in "x" \u2014 "six" stays
  // "six", not "si".
  trimmed = trimmed.replace(/(\d+)\s*x$/i, '$1').trim();
  if (!trimmed) return null;
...
/**
 * Like extractQuantity, but only ever counts a quantity that sits
 * *immediately* at the start of the given text (only leading whitespace
 * allowed in between) \u2014 never one found anywhere later on. Used only for
 * "strict" keywords (the bare color words standing in for a product), so
 * that an unrelated number appearing later in the same message or clause \u2014
 * Thai "555"/"5555" internet-slang laughter, a time, someone else's
 * message \u2014 can never be mistaken for that color word's quantity. Only
 * "\u0e41\u0e14\u0e07 2" / "\u0e41\u0e14\u0e07 x 2" / "\u0e41\u0e14\u0e07\u0e16\u0e31\u0e07\u0e19\u0e36\u0e07" (number right next to the word) count;
 * "\u0e41\u0e14\u0e07...(anything else)...5555" does not.
 */
function extractImmediateQuantity(text) {
  // "20L" / "20 liters" / "20\u0e25\u0e34\u0e15\u0e23" right after the keyword is still just a
  // capacity mention, not a quantity \u2014 strip it before checking.
  let cleaned = text.replace(/^\s*20\s*(l\b|liters?|litres?|\u0e25\u0e34\u0e15\u0e23)/i, '');
  ...
  const bareDigit = cleaned.match(/^(\d+)/);
  if (bareDigit) return toPositiveInt(bareDigit[1]);

  // "\u0e16\u0e31\u0e07\u0e19\u0e36\u0e07" ("a keg"/"one keg") \u2014 strip a leading "\u0e16\u0e31\u0e07" so the "\u0e19\u0e36\u0e07"
  // word-number entry below can still match right at the start.
  const withoutKegPrefix = cleaned.replace(/^\u0e16\u0e31\u0e07\s*/, '');
  const lowerCleaned = cleaned.toLowerCase();

  for (const [word, value] of Object.entries(NUMBER_WORDS)) {
    if (THAI_CHAR_PATTERN.test(word)) {
      if (cleaned.startsWith(word) || withoutKegPrefix.startsWith(word)) return value;
    } else if (new RegExp(`^${word}\\b`, 'i').test(lowerCleaned)) {
      return value;
    }
  }
  return null;
 ...
 * number meant for it.
 *
 * If a product has no number of its own after it, it falls back to looking
 * just before it instead ("1 lager", "one lager" \u2014 the more natural
 * English order), but only when that space isn't already spoken for by the
 * previous product's own number (so "lager 2 rose 3" can't have its "2"
 * double-counted for rose as well as lager). When several such products in
 * a row are joined only by "and"/"\u0e41\u0e25\u0e30"/"\u0e01\u0e31\u0e1a" with nothing else between them
 * ("one of Lager and rose", "\u0e25\u0e32\u0e40\u0e01\u0e2d\u0e23\u0e4c \u0e41\u0e25\u0e30 \u0e42\u0e23\u0e40\u0e0b\u0e48"), a single quantity found
 * before the whole group is shared by every product in it \u2014 that's what
 * "one of Lager and rose" means: one of each, not one lager and an
 * unspecified amount of rose.
 *
 * A product with no quantity found by any of this is dropped entirely
 * rather than defaulting to 1 \u2014 someone just mentioning a product in
 * passing ("\u0e25\u0e32\u0e40\u0e01\u0e2d\u0e23\u0e4c\u0e22\u0e31\u0e07\u0e44\u0e21\u0e48\u0e40\u0e22\u0e47\u0e19", "Rose is no good") isn't placing an order,
...
function matchProductsInClause(clause) {
  ...
  // Pass 1: each occurrence tries to claim a quantity from the text right
  // after it, independently of every other occurrence.
  const quantities = new Array(n).fill(null);
  const matchedForward = new Array(n).fill(false);
  for (let i = 0; i < n; i++) {
    const occ = occurrences[i];
    const next = occurrences[i + 1];
    const segmentEnd = next ? next.start : remainder.length;
    // Strict occurrences (bare color words) only ever accept a quantity
    // sitting immediately next to them \u2014 never one found anywhere later in
    // the rest of the clause \u2014 so they use a separate, anchored extractor
    // over just the text after the keyword itself.
    const q = occ.strict
      ? extractImmediateQuantity(remainder.slice(occ.end, segmentEnd))
      : extractQuantity(remainder.slice(occ.start, segmentEnd));
    if (q !== null) {
      quantities[i] = q;
      matchedForward[i] = true;
    }
  }

  // Pass 2: group up consecutive occurrences that pass 1 left empty-handed,
  // as long as only a bare conjunction sits between each pair in the group
  // (real content in between \u2014 a comma, another number, unrelated words \u2014
  // breaks the group). If a quantity is found before the group starts, and
  // that space wasn't already claimed by the occurrence right before the
  // group, every member of the group gets that same quantity.
  let i = 0;
  while (i < n) {
    if (matchedForward[i]) {
      i++;
    ...
    while (
      j + 1 < n &&
      !matchedForward[j + 1] &&
      isJustConjunction(remainder.slice(occurrences[j].end, occurrences[j + 1].start))
    ) {
      j++;
    }

    if (i === 0 || matchedForward[i - 1] === false) {
      const groupStart = i === 0 ? 0 : occurrences[i - 1].end;
      const preSegment = remainder.slice(groupStart, occurrences[i].start);
      const sharedQuantity = extractPrecedingQuantity(preSegment);
      if (sharedQuantity !== null) {
        // A "strict" occurrence (e.g. a bare color word standing in for a
        // product) only ever accepts a quantity that came from pass 1,
        // directly after it \u2014 never one borrowed from before it or shared
        // across an "and"-joined group \u2014 so it's skipped here even when the
        // rest of the group does get the shared quantity.
        for (let k = i; k <= j; k++) {
          if (!occurrences[k].strict) quantities[k] = sharedQuantity;
        }
      }
    }
